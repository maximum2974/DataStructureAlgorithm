 ...
 * @BelongsPackage: com.maximum.dijkstra
 * @Author: maximum
...
class Graph{
    ...
    public void dsj(int index){
        ...
        for (int j = 0; j < vertex.length; j++) {
            index = vv.updateArr(); //选择并放回新的访问顶点
    ...
    private void update(int index){
        ...
        for (int j = 0; j < matrix[index].length; j++) {
            // len 含义是: 出发顶点到index顶点的距离 + 从index顶点到j顶点距离的和
...
class VisitedVertex{
    ...
    //记录出发顶点到其他所有顶点的距离, 比如G为出发顶点, 就会记录G到其他顶点的距离, 会动态更新, 球的最短距离就会存放到dis
    public int[] dis;
